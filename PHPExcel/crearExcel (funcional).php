...
/************* Nombre Funcionario ********************************************/
$usuario ='Pablo Garcia';
$objSheet->setCellValue('B1', $usuario);
...
$firstrow = 4;
$fecha = $año.'-'.$mes.'-12';
$strMes =strtoupper(strftime("%B",strtotime($fecha)));
$strAnio = strtoupper(strftime(" %Y",strtotime($fecha)));
...
//                }
                ...
                if($diff > $intervalo){
                    ...
                    $lastdate = $fecha;
                        ...
                        $inconsistencias[$cont2] = substr(explode(' ',$dato['CheckTime'])[1],0,-3);
                        $lastdate = $fecha;
                        $fijo[$cont][2] = "Inconsistencia!";
                                        $objSheet->getStyle('A'.($cont+$firstrow-1).':N'.($cont+$firstrow-1))->applyFromArray($pintarInconsistencia);
...
$archivo = "Contralor ".$usuario." ".$strMes.".xls";
$objSheet->setTitle('Usuarios');
$objXLS->setActiveSheetIndex(0);
$objXLS->getProperties()->setTitle("Contralor ".$usuario." ".$strMes);
