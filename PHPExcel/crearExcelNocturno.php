...
$ultimo_dia = cal_days_in_month(CAL_GREGORIAN, $mes, $año);
$firstrow = 4;
$strfecha = $año.'-'.$mes.'-12'; //para el uso que se le dara el dia es indiferente
...
/**************************** Filtro de marcas menores a un intervalo dado ***************************************************************/
                ...
                if($diferencia > $intervalo){// si la diferencia con la ultima fecha registrada es mayor al intervalo minimo se continua sino se omite la marca
													...
													$inconsistencias[$cont2] = substr(explode(' ',$dato['datetime'])[1],0,-3).' '.$type;
													//$lastdate = $fecha;
													$lastdate = $date;
...
/***************************************************************************************************************/
   ...
   // $objXLS->getActiveSheet()->fromArray($total, NULL, 'H'.$firstrow );
	...
	//	$contOut = 0
		...
		//$objSheet->setCellValue('H'.($cont+$firstrow-1), '=SUM(E'.($cont+$firstrow-1).'-D'.($cont+$firstrow-1).'+A50)'); //=SI(E12=D12;"00:00";SUMA(E12-D12+TEXTO(1;"[hh]:mm:ss")))
		
		$objSheet->setCellValue('H'.($cont+$firstrow-1), '=IF(E'.($cont+$firstrow-1).'=D'.($cont+$firstrow-1).',"00:00",SUM(E'.($cont+$firstrow-1).'-D'.($cont+$firstrow-1).'+TEXT(1,"[hh]:mm:ss")))');
		//+TEXT(1,"[hh]:mm:ss") -> Esto le suma 24 Horas para calcular las horas trabajadas correctamente
		// Si la entrada y salidas son iguales escribe: 00:00, esto es para que no escriba  24:00 si no trabajo ese dia, es decir si entrada y salida son 00:00. Si no son 00:00 las dos pero son iguales tambien marca 00:00.
		 

		//echo 'H'.($cont+$firstrow-1), '=IF(E'.($cont+$firstrow-1).'=D'.($cont+$firstrow-1).',"00:00",SUM(E'.($cont+$firstrow-1).'-D'.($cont+$firstrow-1).'+TEXT(1,"[hh]:mm:ss")))</br>';
    if(isset($marcas)){
